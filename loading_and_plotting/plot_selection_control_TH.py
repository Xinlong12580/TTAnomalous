...
with open("pkls/hists_selection_control_TH.pkl", "rb") as f:
    hists = pickle.load(f)
...
bins = {}
bin_centers = {}
bins["Mass_GJ"] = array.array("d", np.linspace(0, 2000, 201))

bins["Phi_GJ"] = array.array("d", np.linspace(-np.pi, np.pi , 21) )

bins["Eta_GJ"] = array.array("d", np.linspace(-3, 3, 21) )

bins["Pt_GJ"] = array.array("d", np.linspace(0, 2000, 201) )

bins["Tagger_GJ"] = array.array("d", np.linspace(0, 1, 101) )
...
years = [ "2024"]
processes = { "MC_QCDJets": ["*"]}
save_dir = "plots/plots_selection_control_TH"
#-------------------------------------rebinning -----------------------------------------
rebinned_h_data = {}
rebinned_h_BKGs = {}
for year in h_data:
    ...
    for column in h_data[year]:
        ...
        print(h_data[year][column].Integral())
for year in h_BKGs:
...
for year in years:
    ...
    for column in bins:
        ...
        ratio_error[year][column] = [ max(0, x / y) for x,y in zip(data_binned_error[year][column], values)]

...
for year in years:
    for column in bins:
        ...
        scale_factor = (sum(data_binned[year][column]) - h_TTBar[year][column].Integral() )/ h_QCD[year][column].Integral()
        h_QCD[year][column].Scale(scale_factor)  
        colors = []
        labels = []
        hs = []
        ...
        fig.savefig(f"{save_dir}/rescaled_linear_stack_{year}_{column}.png")
        ax1.set_yscale("log")
        ax1.set_ylim(1,10000000)
        fig.savefig(f"{save_dir}/rescaled_stack_{year}_{column}.png")

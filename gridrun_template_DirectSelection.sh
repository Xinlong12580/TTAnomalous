...
python run_skim.py $1 $2 $3 $4 $5 $6 $7 $8 2>&1 | tee -a $root_dir/$OUTTXT
out_f=$(ls *skimmed*root)
echo python run_selection_combined_BDT.py -d $out_f -y $4 -n 1 -i 0 $9 ${10} ${11} ${12}  | tee -a $root_dir/$OUTTXT
python run_selection_combined_BDT.py -d $out_f -y $4 -n 1 -i 0 $9 ${10} ${11} ${12} 2>&1 | tee -a $root_dir/$OUTTXT
status=${PIPESTATUS[0]}
if [ $status -eq 0 ]; then
    # move all snapshots to the EOS (there will only be one)
    xrdcp -f Reg*.root OUTPUT_DIR
else
